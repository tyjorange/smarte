 ...
 */
@Component
public class CollectorCommandExecutor {
     ...
     * 命令执行线程池。
     */
//	private ExecutorService executorService = VirtualFsuUtil.executorService;
            ...
            for (int i = 0;
                ...
                if (ctx != null) {    // 集中器在线
                    ...
                    if (stopFlag)
                        break;
                    try {
                        s = VirtualFsuUtil.virtualFsuService.findSwitchBySwitchID(breakerCommand.getBreakerId());    // 查询断路器最新记录
                    } catch (Exception e) {
                        s = null;
                    }
                    if (s == null) {
...
//            CompletableFuture.supplyAsync(() -> doBreakerCommand(breakerCommand), executorService)    // 异步执行
//                    .thenAcceptAsync(this::doBreakerCommandResult, executorService);    // 执行完毕后异步处理
            this.doCommand(breakerCommand);
        }
    ...
    @Async
    BreakerCommand doCommand(BreakerCommand breakerCommand) {
        doBreakerCommand(breakerCommand);
        doBreakerCommandResult(breakerCommand);
        return breakerCommand;
    }
