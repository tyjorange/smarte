...
public class BreakerTimerExecutor {
	...
	public BreakerTimerExecutor(ScheduledExecutorService scheduledExecutorService) {
							...
							if(!stopFlag && !Thread.currentThread().isInterrupted()) {
								...
								if(minuteTimers != null) {
									...
									if(ids.length() > 2) {	// "()"表示没有定时记录。
										List<TimeController> tcs = VirtualFsuUtil.virtualFsuService.findTimeControllerByIDList(ids);
										if(tcs != null && !tcs.isEmpty())
											tcs.forEach(tc -> doTimer(tc));
									}
								}
							}
						} catch (Exception e) {
	...
	private int loadTimers() {
		int count = 0;
		try {
			List<TimeController> timeControllers = VirtualFsuUtil.virtualFsuService.findAllEnabledTimeController();
	...
	private void doTimer(TimeController timeController) {
		try {
			...
			if((weekDay == 0) ||
				...
				VirtualFsuUtil.virtualFsuService.doTimeBreakerController(timeController);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
